...
var goodsController = {
    init: function () {
        goodsController.loadData();
        goodsController.registerEvent();
    },
    registerEvent: function () {
        ...
        $('.txtSalary').off('keypress').on('keypress', function (e) {
            if (e.which == 13) {
                ...
                goodsController.updateSalary(id, value);
            }
        });
        $('#txtNameS').off('keypress').on('keypress', function (e) {
            if (e.which == 13) {
                goodsController.loadData(true);
            }
        });
        ...
        $('#btnAddNew').off('click').on('click', function () {
            ...
            goodsController.resetForm();
        });

        $('#btnSave').off('click').on('click', function () {
            if ($('#frmSaveData').valid()) {
                goodsController.saveData();
            }
        });
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    $('#tblData').html(html);
                    goodsController.paging(response.total, function () {
                        goodsController.loadData();
                    }, changePageSize);
                    goodsController.registerEvent();
                }
